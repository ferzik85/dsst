		...
		if (first == true) {
			for (int i = 0; i < dout; i++)
				for (int j = 0; j < prodszhalf; j++) {
					...
					hf_num[i][j][0] = yf[j][0] * xtf[i][j][0] + yf[j][1] * xtf[i][j][1];
					hf_num[i][j][1] = -yf[j][0] * xtf[i][j][1] + yf[j][1] * xtf[i][j][0];
				}

			memset(hf_den, 0, sizeof(float) * prodszhalf);
			for (int i = 0; i < dout; i++)
				for (int j = 0; j < prodszhalf; j++){
					//new_hf_den = sum(xtf .* conj(xtf), 3) <-- idea 
					hf_den[j] += xtf[i][j][0] * xtf[i][j][0] + xtf[i][j][1] * xtf[i][j][1];
				}
		}
		else {
			for (int i = 0; i < dout; i++)
				for (int j = 0; j < prodszhalf; j++) {
					//new_hf_num[i][j] = yf[j] * conj(xtf) <-- idea 
					hf_num[i][j][0] = (1 - learning_rate) * hf_num[i][j][0] + learning_rate * (yf[j][0] * xtf[i][j][0] + yf[j][1] * xtf[i][j][1]);
					hf_num[i][j][1] = (1 - learning_rate) * hf_num[i][j][1] + learning_rate * (-yf[j][0] * xtf[i][j][1] + yf[j][1] * xtf[i][j][0]);
				}
			...
			for (int j = 0; j < prodszhalf; j++) {
				...
				for (int i = 0; i < dout; i++) {
					//new_hf_den = sum(xtf .* conj(xtf), 3) <-- idea 
					sum_d += xtf[i][j][0] * xtf[i][j][0] + xtf[i][j][1] * xtf[i][j][1];
				}
				hf_den[j] = (1 - learning_rate) * hf_den[j] + learning_rate * sum_d;
			}
		}
