  ...
  int bin, int nOrients, int softBin, bool full )
{
  ...
  for( x=0; x<w0; x++ ) {
	...
	else { // NOT TESTED WITHOUT SSE !!!! Падает при выключеном ссе при удалении массива Н
	...
	int nOrients, int softBin, float clip)
{
	...
	R1 = new float[wb*hb*nOrients * 3];	//3 чтобы не падало из-за обращения в ячейку для которой не выделена память , а так должно быть 2
